...
root = pathlib.Path('/dataHDD/chsaikeerthi/2024-chirumamilla/data/fda_eval')
README = (root / 'README').read_text()
sentences = {int(k): v for k, v in [l.strip().split(' ', 1) for l in (root / 'orthographic.index').open()]}

dataset = jbof.create_dataset('/dataHDD/chsaikeerthi/2024-chirumamilla/data/FDA', {'README': README})
...
for file in itertools.chain(root.glob('rl/*.sig'),
    ...
    new_laryngograph_samplerate = 16_000
    item = dataset.add_item(file.stem, metadata)
    item.add_array('pitch', pitch, pitch_metadata)
    soundfile.write('signal.wav', speech, new_samplerate)
    item.add_array_from_file('signal', 'signal.wav')
    soundfile.write('laryngograph.wav', laryngograph, new_laryngograph_samplerate)
